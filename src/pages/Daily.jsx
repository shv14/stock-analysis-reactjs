...
// const Daily = () => {
//     return (
//         <div className="daily-page p-4 min-h-screen flex flex-col items-center justify-center bg-cover bg-center">
//             <h2 className="text-center text-3xl font-bold mb-4">Daily Stock Prices</h2>
...
// import React from 'react';
// import dailyImage from '../images/daily.jpg'; // Import your image here

// const Daily = () => {
//     return (
//         <div className="relative w-full h-screen bg-cover bg-center" style={{ backgroundImage: `url(${dailyImage})` }}>
//             <div className="absolute inset-0 bg-black bg-opacity-50 p-8">
//                 <h1 className="text-3xl md:text-5xl font-bold mb-4">Daily Stock Data</h1>
//                 <div className="bg-gray-700 bg-opacity-70 backdrop-blur-md p-6 rounded-lg">
//                     {/* Add your stock data components here */}
//                 </div>
//             </div>
//         </div>
//     );
...
//             setError('');
//             try {
//                 const response = await fetch(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&outputsize=full&apikey=demo`);
...
//                                 <div key={date} className="mb-4 bg-gray-800 bg-opacity-80 p-4 rounded-lg">
//                                     <h2 className="text-lg font-bold">{date}</h2>
//                                     <p>Open: {data[date]['1. open']}</p>
//                                     <p>High: {data[date]['2. high']}</p>
//                                     <p>Low: {data[date]['3. low']}</p>
//                                     <p>Close: {data[date]['4. close']}</p>
//                                     <p>Volume: {data[date]['5. volume']}</p>
//                                 </div>
//                             ))}
//                         </div>
//                     )}
...
const Daily = () => {
    ...
    const fetchData = async () => {
        ...
        setError('');
        try {
            const response = await fetch(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&outputsize=compact&apikey=demo`);
            const result = await response.json();
            if (result['Time Series (Daily)']) {
                setData(result['Time Series (Daily)']);
            } else {
    ...
    return (
        <div className="relative w-full h-screen bg-cover bg-center" style={{ backgroundImage: `url(${dailyImage})` }}>
            <div className="absolute inset-0 bg-black bg-opacity-50 p-8">
            <h1 className=" flex text-3xl md:text-5xl font-bold m-5">Daily <p className="mx-1" style={{color: 'cyan'}}>Stock</p> Data</h1>
                <div className="bg-gray-700 bg-opacity-70 backdrop-blur-md p-6 rounded-lg">
                    <form onSubmit={handleSubmit} className="mb-6">
                        <input
                            type="text"
                            ...
                            onChange={handleChange}
                            placeholder="Enter stock symbol"
                            className="px-4 py-2 rounded-lg w-full mb-4 outline-none text-black"
                        />
